---
title: 邮件提供商
description: 在 Admin9 Filament 管理后台配置邮件提供商资源的运营指引。
---

当需要在发布前启用、修复或验证外发邮件能力时，请使用本指南。
...
如果该开关关闭，provider、SMTP 以及其他后台设置页面将拒绝访问。
...
运营人员在此配置内置 provider，不在该页面创建或删除 provider 记录。
...
由于该服务同时写入数据库和缓存，更新 provider 凭据后通常无需重启应用；但在依赖新值前，仍应确认其已在 Filament UI 和 `config('services.<provider>')` 中生效。
